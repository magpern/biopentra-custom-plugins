...
mkdir -p "${DISPOSABLE}/releases/upr-host-adapter/0.1.4" \
	"${DISPOSABLE}/releases/universal-product-reviews/0.2.9"
echo "previous-host" > "${DISPOSABLE}/releases/upr-host-adapter/0.1.4/marker.txt"
echo "previous-upr" > "${DISPOSABLE}/releases/universal-product-reviews/0.2.9/marker.txt"
# Manifests for previous trees
( cd "${DISPOSABLE}/releases/upr-host-adapter/0.1.4" && sha256sum marker.txt > "../0.1.4.SHA256SUMS" )
...
ln -sfn "${DISPOSABLE}/releases/upr-host-adapter/0.1.4" "${DISPOSABLE}/plugins/upr-host-adapter"
...
ln -sfn "${DISPOSABLE}/releases/upr-host-adapter/0.1.4" "${DISPOSABLE}/pointers/upr-host-adapter/current"
...
readlink -f "${DISPOSABLE}/plugins/upr-host-adapter" | grep -q '/0.1.4$'
