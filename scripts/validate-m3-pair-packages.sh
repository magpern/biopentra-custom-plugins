#!/usr/bin/env bash
#
# Disposable (non-WordPress) validation of the M3 UPR+host package pair.
...
#   HOST_ZIP — path to biopentra-upr-host-0.1.5.zip
# Optional:
...
verify_sum() {
	...
	if [[ -n "${sums}" && -f "${sums}" ]]; then
		local zip_base sums_dir digest
...
HOST_DIR="$(find "${DISPOSABLE}/extract/host" -maxdepth 2 -type d -name 'biopentra-upr-host' | head -n1)"
