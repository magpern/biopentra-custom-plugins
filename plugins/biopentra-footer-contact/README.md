...
| Item | Location |
|------|----------|
| **Replacement module** | `plugins/biopentra-storefront/modules/footer-contact/` |
| **Releases** | `storefront-v*` tags on this monorepo only |
| **Release ZIP** | `biopentra-storefront-{version}.zip` |

## Policy

- The slug `biopentra-footer-contact` is **intentionally excluded** from monorepo release tooling (`RELEASE_SKIP_PLUGINS`).
- **Do not edit source here.** New changes belong in the storefront footer-contact module.
...
Shortcode `[biopentra_footer_email]`, placeholder-page `wp_robots` noindex, JS-built mailto (no address in HTML).
