...
final class Biopentra_Upr_Host_Support_Adapter {
	...
	public static function filter_invitation_action( array $decision, int $order_id, int $order_item_id ): array {
		...
		$tags = $signals['tags'];
		foreach ( Biopentra_Upr_Host_Options::support_suppress_tags() as $suppress_tag ) {
		...
		if ( $has_open ) {
			foreach ( Biopentra_Upr_Host_Options::support_delay_tags() as $delay_tag ) {
	...
	private static function delay_decision( string $code ): array {
		$days = Biopentra_Upr_Host_Options::support_delay_days();
		return array(
			'action'      => 'delay',
			'code'        => $code,
	...
	private static function lookup_structured_signals( int $order_id ): ?array {
		...
		if ( $tickets === $ticket_table ) {
			$open_statuses = Biopentra_Upr_Host_Options::support_open_statuses();
			if ( ! empty( $open_statuses ) ) {
				...
				foreach ( $submission_ids as $submission_id ) {
					...
					if ( false === $found && null === $found ) {
						// Distinguish empty vs error: get_var returns null when no row.
					}
					if ( $wpdb->last_error ) {
						return null;
					}
