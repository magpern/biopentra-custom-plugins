...
final class Biopentra_Upr_Host_Delivery_Adapter {

	public const META_HOST_CONFIRMED_AT = '_biopentra_upr_host_delivery_confirmed_at';
	public const META_HOST_SOURCE       = '_biopentra_upr_host_delivery_source';
	public const META_HOST_FULFILLMENT  = '_biopentra_upr_host_fulfillment_id';
	...
	public static function on_fulfillment_state_changed( $payload ): void {
		try {
			...
			if ( 'shipped' === $to_state && Biopentra_Upr_Host_Options::confirm_on_shipped() ) {
	...
	private static function log_error( string $message ): void {
		if ( function_exists( 'wc_get_logger' ) ) {
			wc_get_logger()->error( $message, array( 'source' => 'biopentra-upr-host' ) );
			return;
		}
		// phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log
		error_log( '[biopentra-upr-host] ' . $message );
	}
}
