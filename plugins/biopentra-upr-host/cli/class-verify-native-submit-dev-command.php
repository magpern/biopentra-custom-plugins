...
final class Biopentra_Upr_Host_Verify_Native_Submit_Dev_Command {
	...
	private function assert_host_does_not_own_security_hooks(): void {
		$comments_open = has_filter(
			'comments_open',
			array( 'Biopentra_Upr_Host_Review_Availability_Ux', 'maybe_close_product_reviews' )
		);
		...
		$preprocess = has_filter(
			'preprocess_comment',
			array( 'Biopentra_Upr_Host_Review_Availability_Ux', 'reject_unavailable_native_product_review' )
		);
