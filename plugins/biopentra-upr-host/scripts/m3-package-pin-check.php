<?php
/**
 * Offline package-pin checks for biopentra-upr-host 0.1.5+ (no WordPress bootstrap).
 *
...
namespace {
	...
	function write_meta( ?array $meta ): void {
		$path = rtrim( (string) UPR_PLUGIN_DIR, '/' ) . '/' . \Biopentra_Upr_Host_Upr_Pin::PACKAGE_META_BASENAME;
		if ( null === $meta ) {
			if ( is_file( $path ) ) {
				unlink( $path );
			}
	...
	$child = escapeshellarg( PHP_BINARY ) . ' ' . escapeshellarg( __DIR__ . '/m3-package-pin-check-missing-api.php' );
