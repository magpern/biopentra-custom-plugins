...
**Status:** **Plan frozen for implementation authorization**
...
**Goal.** Make the storefront feel slightly more modern through restrained, reusable, homepage-first motion owned by `biopentra-storefront`, without harming accessibility, performance, SEO, or frozen mobile-first redesign baselines.

**Non-goals**

- Not M11. Not a reopen of M1–M10, PDP-1, header, footer, shop discovery, or contact.
...
- No shop, search, SEO landing, WooCommerce archive, related/upsell, or infinite-scroll **product-card** reveal or stagger.
...
- Shop/search/SEO/archive cards receive **no** `data-bp-motion*` attributes.
...
**Shop dynamic grids.** `biopentra-loop-card` `assets/shop-loop-filter.js` replaces/appends loop `ed52b7f` and fires `biopentra-loop-cards-init`. MOTION-1 must ignore those cards.
...
- Shop `ed52b7f`, search results, SEO `.bp-seo-grid-section`, WC archives, related/upsell per-card motion.
...
- `data-bp-motion-state="pending|in"` — on nodes that hide/reveal: the section band **or** curated-grid **cards**. Never shop/search/SEO/archive cards.
...
Frontend only. Skip `is_admin()`, cart, and checkout (`is_cart()`, `is_checkout()`). Do not load on wp-login.
...
1. Classified every eligible **initial** homepage node.
2. Configured the single `IntersectionObserver` and `observe()`’d every off-screen section and every off-screen curated-grid **container**.
...
- **One** `IntersectionObserver` observes `.bp-home-products-section` **containers** only. **Never** observe individual cards. **Never** observe shop results.
...
- Shop/search/archive/SEO cards: **unstamped**.
...
Track: one observer; observed sections/containers; WeakMap pending node → timeout id.

| Event | Actions |
|---|---|
| Reveal (observer / bulk `in`) | Set nodes `in`; `clearTimeout` + delete map; `unobserve` that section or container |
| Per-node timer | Set that node `in`; clear its timer. Section: `unobserve` if observed. Card: if **all** eligible cards in that grid are `in`, `unobserve(container)` |
...
`biopentra-loop-cards-init`: **do not** stamp shop/search cards.
...
2. Homepage-first: section reveal + curated **card** stagger only.
3. No hero; no shop/search/SEO/archive/infinite-scroll card motion.
...
| WP5 | Shop contract: no stamps on `ed52b7f` / `biopentra-loop-cards-init` | Filter + load-more still work |
...
2. Shop/search: filter + infinite-scroll functional; those cards have no motion attributes.
...
- One observer; compositor-only reveal; no per-card observers on shop.
